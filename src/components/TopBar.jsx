...
export function TopBar({ notifOpen, setNotifOpen, notifications, onCreateClick, onSearchClick, markRead }) {
  ...
  return (
    <div ref={notifRef} className="relative flex items-center justify-between px-4 pt-4 pb-3 bg-zinc-950 border-b border-zinc-900">
      <Logo />
      <div className="flex items-center gap-1">
        <button aria-label="Create community" onClick={onCreateClick} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-zinc-900">
        ...
        <button aria-label="Notifications" onClick={toggleNotifs} className="relative w-9 h-9 flex items-center justify-center rounded-full hover:bg-zinc-900">
          ...
          {unreadCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-blue-400 ring-2 ring-zinc-950" />}
        </button>
      </div>
      {notifOpen && (
          ...
          <div className="max-h-72 overflow-y-auto no-scrollbar">
            {notifications.map((n) => (
              <div key={n.id} className="px-4 py-3 border-b border-zinc-800/60 last:border-0 flex gap-2 items-start">
                {n.unread && <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />}
