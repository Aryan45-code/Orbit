...
import {
  ArrowLeft, Settings, TrendingUp, Crown, MapPin, Flame, Search, Pin,
  Image as ImageIcon, Camera, Flag, Check, X, Trash2,
} from "lucide-react";
import { CATEGORIES, COLOR_MAP, MOCK_SIMILAR_PEOPLE } from "../data/constants.js";
import { distanceKm, baseSparks, genMembers, nextId } from "../utils/helpers.js";
    ...
    <div className="absolute inset-0 bg-black/60 z-[70] flex items-end" onClick={onClose}>
      <div className="bg-zinc-950 w-full rounded-t-3xl max-h-[92%] overflow-y-auto no-scrollbar border-t border-zinc-800" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pb-6">
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-xl px-3.5 py-2.5 text-sm mb-4 outline-none focus:border-blue-500" />
          <p className="text-xs text-zinc-500 mb-1.5">Description</p>
          <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={3} className="w-full bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-xl px-3.5 py-2.5 text-sm mb-4 outline-none focus:border-blue-500" />
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            {CATEGORIES.map((cat) => {
              ...
              const Icon = cat.icon;
              return (
                <button key={cat.name} onClick={() => setCategory(cat.name)} className={`flex items-center gap-2 rounded-xl px-3 py-2.5 text-left border ${selected ? "border-blue-500 ring-1 ring-blue-500/30 bg-blue-500/5" : "border-zinc-800 bg-zinc-900"}`}>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-blue-500 disabled:bg-zinc-800 text-white disabled:text-zinc-500 text-sm font-semibold mb-3"
          >
            Save changes
          </button>
          {!confirmDelete ? (
        ...
        <div className="px-5 pt-3">
          <div className="mb-1 flex items-end justify-between">
            ...
            <button
              ...
              className={`mb-1 px-5 py-2.5 rounded-xl text-sm font-semibold shrink-0 ${joined ? "bg-zinc-800 text-zinc-300 border border-zinc-700" : "bg-blue-500 text-white"}`}
            >
              {joined ? "Joined ✓" : "Join"}
            </button>
          </div>
          <div className="flex items-center gap-2 mb-2 mt-3 flex-wrap">
            ...
            {isAdmin && <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-500/10 text-blue-300"><Crown size={11} />You created this</div>}
          </div>
          <h2 className="text-lg font-bold text-zinc-50">{c.name}</h2>
          <p className="text-sm text-zinc-400 mt-1.5">{c.desc}</p>
          <div className="flex items-center gap-3 mt-3 text-xs text-zinc-500">
            <span className="flex items-center gap-1 mono text-blue-400"><MapPin size={13} />{distanceKm(c.dx, c.dy).toFixed(1)} km away</span>
          ...
          {matchedMembers.length > 0 && (
            <div className="mt-3 flex items-center gap-2 bg-blue-500/5 border border-blue-500/20 rounded-xl px-3 py-2">
            ...
            {joined && (
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-3 mb-4">
                ...
                <div className="flex items-center justify-between mt-1.5">
                  <button onClick={() => setAttaching((v) => !v)} className={`w-8 h-8 rounded-lg flex items-center justify-center ${attaching ? "bg-blue-500/15 text-blue-400" : "text-zinc-500 hover:bg-zinc-800"}`}>
                    <Camera size={16} />
                  </button>
                  <button disabled={!draft.trim()} onClick={() => handlePost()} className="px-4 py-1.5 rounded-lg bg-blue-500 disabled:bg-zinc-800 text-white disabled:text-zinc-500 text-xs font-semibold">Post</button>
                </div>
              </div>
            )}
                  ...
                  <div key={p.id} className={`flex gap-2.5 ${isPinned ? `${cm.tint} border border-zinc-800/60 -mx-2 px-3 py-2.5 rounded-xl` : ""}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1.5">
                        ...
                        {isAdmin && (
                          <button onClick={() => setPinnedId((cur) => (cur === p.id ? null : p.id))} className="text-[11px] text-zinc-500 hover:text-blue-400 flex items-center gap-0.5">
                  ...
                  <div key={i} className="flex items-center gap-3 py-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-zinc-500 flex items-center gap-1">
                        ...
                        {overlap.length > 0 && <span className="text-blue-400">· shares {overlap.join(", ")}</span>}
                      </p>
                    </div>
